...
fn graph_bench_point( mut graph:  WaveletTreeGraph<WaveletTreePointer<u64>>) {
    for i in 0..300 {
        graph.neighbor(0, 1);
    }
}
...
fn bench_graph_neighbor(c: &mut Criterion) {
    ...
    let mut graphP = wt_graph_builder.to_graph::<WaveletTreePointer<u64>>();
    let mut graphC = wt_graph_builder.to_graph::<WaveletTreeCompact<u64>>();


    c.bench(
        "Graph",
        Benchmark::new("Compact_neig", | b | b.iter(|| graph_bench_point(graphP ))),
    );
}
